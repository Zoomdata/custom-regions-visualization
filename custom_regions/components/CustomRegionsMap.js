...
(function() {
    ...
    */
    ...
    //Omnivore is not loading in Microsoft Edge from the libraries collection in the custom chart.
    //try loading as a script tag
    if (typeof(omnivore) === 'undefined' || !omnivore) {
        var omnivoreScriptBlock = document.createElement('script');
        omnivoreScriptBlock.src = 'https://api.tiles.mapbox.com/mapbox.js/plugins/leaflet-omnivore/v0.3.1/leaflet-omnivore.min.js';
        omnivoreScriptBlock.type = 'text/javascript';
        omnivoreScriptBlock.onload = function() {
            console.log('Omnivore loaded ', typeof(omnivore));
            initChart();
        };
        controller.element.appendChild(omnivoreScriptBlock);
    } else {
        initChart();
    }
    ...
    function setLayerFilter() {
        ...
        if (currFilters.length !== 0) {
            userVariables.regionsConfig.forEach(function(r) {
                ...
                //Find method not supported in IE
                ...
                for(var i=0; i < currFilters.length; i++) {
                    if(currFilters[i].path === r.groupName) {
                        controller.query.filters.removeFilters(currFilters[i]);
                        matchingFilter = currFilters[i];
                    }
                }
            });
        }
    ...
    function style(feature) {
        ...
        //});
        var visibleRegion = getVisibleLayer();
